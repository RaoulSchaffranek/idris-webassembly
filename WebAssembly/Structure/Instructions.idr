...
mutual
  total public export
  instrsMaybeEqual  : (is : List Instr) -> (js : List Instr) -> Maybe (is = js)
  instrsMaybeEqual [] [] = Just Refl
  instrsMaybeEqual [] (x :: xs) = Nothing
  instrsMaybeEqual (x :: xs) [] = Nothing
  instrsMaybeEqual (x :: xs) (y :: ys) = case instrMaybeEqual x y of
    Nothing   => Nothing
    Just Refl => case instrsMaybeEqual xs ys of
      Nothing   => Nothing
      Just Refl => Just Refl
  ...
  instrMaybeEqual (Block x is)         (Block y js)   = case decEq x y of
    No contra => Nothing
    Yes Refl  => case instrsMaybeEqual is js of
      Nothing   => Nothing
      Just Refl => Just Refl
  instrMaybeEqual (Loop x is)          (Loop y js)    = case decEq x y of
    No contra => Nothing
    Yes Refl  => case instrsMaybeEqual is js of
      Nothing   => Nothing
      Just Refl => Just Refl
  instrMaybeEqual (If x is1 js1)    (If y is2 js2)    = case decEq x y of
    No contra => Nothing
    Yes Refl  => case instrsMaybeEqual is1 is2 of
      Nothing   => Nothing
      Just Refl => case instrsMaybeEqual js1 js2 of
        Nothing   => Nothing
        Just Refl => Just Refl
